...
function dirname(p) {
  if (!p) return '';
  return p.replace(/\\/g, '/').split('/').slice(0, -1).join('/') || p;
}
function isAnchoredFile(filePath) {
  return appState.anchored.some(a => filePath.replace(/\\/g, '/').startsWith(a.replace(/\\/g, '/')));
}
...
async function init() {
  ...
  if (state.modsFolder) {
    $('#modsPathDisplay').textContent = state.modsFolder;
  }
  // 同步严格模式开关状态
...
function bindTopbar() {
  $('#btnSelectFolder').addEventListener('click', async () => {
    ...
    $('#modsPathDisplay').textContent = result.path;
    toast('文件夹已选择：' + result.path, 'success');
    renderOverview();
  });
...
async function runDeepScan() {
  if (!appState.scanResults && !$('#modsPathDisplay').textContent.includes(':')) {
...
async function renderAnchoredList() {
  ...
  list.innerHTML = anchored.map(a => {
    const rel = appState.scanResults ? a.replace(appState.scanResults.root, '').replace(/^[\\\/]/, '') : basename(a);
    return `
      <div class="folder-item">
...
async function scanConflicts() {
  ...
  btn.disabled = true;
  $('#conflictSummary').innerHTML = '<span class="loading"></span> 扫描中...';
  try {
    const result = await api.scanConflicts();
    if (result.error) { toast(result.error, 'error'); $('#conflictSummary').textContent = result.error; return; }
    conflictList = result.conflicts;
    renderConflicts();
    $('#conflictSummary').textContent = `发现 ${result.total} 个冲突`;
    toast('冲突检测完成', 'success');
...
function renderConflicts() {
  ...
  area.innerHTML = conflictList.map(c => {
    ...
    return `
    <div class="conflict-item ${c.hasAnchored ? 'has-anchored' : ''}">
      <div class="conflict-header">
        <span class="conflict-detail">${esc(c.detail)}</span>
        ...
        ${c.files.map((f, idx) => `
          <div class="conflict-file">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div class="folder-path">${esc(f.path)} ${f.mtime ? ' · ' + fmtDate(f.mtime) : ''}</div>
            </div>
            ${f.author && f.author !== '未知' ? `<span class="author-badge" title="作者">👤 ${esc(f.author)}</span>` : ''}
            ${f.anchored ? '<span class="anchor-badge">已锚定</span>' : ''}
            <button class="locate-btn" data-locate="${esc(f.path)}" title="定位">📂</button>
            <button class="delete-conflict-btn" data-delconflict="${esc(f.path)}" ${f.anchored ? 'disabled title="已锚定不可删除"' : 'title="删除该文件（若是独立文件夹一并删除）"'}>删除该文件</button>
          </div>
        `).join('')}
      </div>
    </div>
  ...
  area.querySelectorAll('[data-delconflict]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const confirmMsg = `是否确认删除该文件？此操作不可撤销。\n\n${filePath}`;
      ...
      toast(`删除成功${r.deletedFolder ? '（含所在文件夹）' : ''}`, 'success');
      ...
      for (let i = 0; i < conflictList.length; i++) {
        ...
        if (c.files.length <= 1) {
          // 冲突组不再是冲突
          const el = area.querySelectorAll('.conflict-item')[i];
          if (el) el.remove();
