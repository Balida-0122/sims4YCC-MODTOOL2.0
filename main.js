...
// ============ 功能一：预览图与资源类型明细 ============
// Sims4 缩略图资源 Type ID (THUM)
const THUM_TYPES = new Set(['6373436b', '8c3ce7e4', '3f41c21b', '6b437363', 'e4e73c8c']);
// 资源类型 → 中文名称映射（用于明细展示）
...
function resourceTypeName(typeHex) {
  return RESOURCE_TYPE_NAMES[typeHex] || `自定义资源(0x${typeHex})`;
}

// 从 .package buffer 中提取第一张 THUM 缩略图（返回 base64 dataURL 或 null）
function extractThumbnailFromBuffer(buffer) {
  try {
    const parsed = parsePackage(buffer);
    if (!parsed || !parsed.entries || parsed.entries.length === 0) return null;
    for (const e of parsed.entries) {
      if (THUM_TYPES.has(e.type)) {
        const start = e.offset;
        if (!start || start + 8 > buffer.length) continue;
        // THUM 内部可能是 DDS 或 PNG/JPEG 数据；尝试直接取原始字节
        // 先检测是否为 PNG
        let end = buffer.length;
        // 查找下一个条目偏移作为结束边界
        let nextOffset = buffer.length;
        for (const e2 of parsed.entries) {
          if (e2.offset > start && e2.offset < nextOffset) nextOffset = e2.offset;
        }
        end = Math.min(nextOffset, start + 4 * 1024 * 1024);
        const raw = buffer.slice(start, end);
        // 尝试 PNG
        if (raw.length > 8 && raw[0] === 0x89 && raw[1] === 0x50 && raw[2] === 0x4e && raw[3] === 0x47) {
          // 找到 PNG IEND 结束块
          const iend = raw.indexOf(Buffer.from([0x49, 0x45, 0x4e, 0x44, 0xae, 0x42, 0x60, 0x82]));
          const pngEnd = iend >= 0 ? iend + 8 : raw.length;
          return `data:image/png;base64,${raw.slice(0, pngEnd).toString('base64')}`;
        }
        // 尝试 JPEG
        if (raw.length > 2 && raw[0] === 0xff && raw[1] === 0xd8) {
          const jpgEnd = raw.indexOf(Buffer.from([0xff, 0xd9]));
          const end2 = jpgEnd >= 0 ? jpgEnd + 2 : raw.length;
          return `data:image/jpeg;base64,${raw.slice(0, end2).toString('base64')}`;
        }
        // DDS 格式：转换不现实，直接返回原始 DDS 数据作为占位（浏览器不支持 DDS）
        // 返回 null 让上层回退到同目录图片
        return null;
      }
    }
...
// 获取 .package 资源类型统计
function getResourceTypeStats(buffer) {
  try {
    const parsed = parsePackage(buffer);
    if (!parsed || !parsed.entries) return { types: [], total: 0 };
    const typeCount = {};
    for (const e of parsed.entries) {
      typeCount[e.type] = (typeCount[e.type] || 0) + 1;
    }
    const types = Object.entries(typeCount)
      .map(([type, count]) => ({ type, name: resourceTypeName(type), count }))
      .sort((a, b) => b.count - a.count);
    return { types, total: parsed.entries.length };
  } catch (e) {
    return { types: [], total: 0 };
  }
}

ipcMain.handle('get-mod-detail', async (event, filePath) => {
  if (!filePath) return { error: '未提供文件路径' };
  const result = { path: filePath, thumbnail: null, thumbnailSource: 'none', resourceTypes: [], resourceTotal: 0 };
  try {
    ...
    if (ext === '.package') {
      ...
      const thumb = extractThumbnailFromBuffer(buf);
      if (thumb) {
        result.thumbnail = thumb;
        result.thumbnailSource = 'package';
      } else {
        // 回退：扫描同文件夹图片
        const dir = path.dirname(filePath);
        let entries;
        ...
        for (const e of entries) {
          if (e.isFile() && isImageFile(e.name)) {
            const imgPath = path.join(dir, e.name);
            try {
              const data = await fsp.readFile(imgPath);
              const imgExt = path.extname(imgPath).slice(1).toLowerCase();
              const mime = imgExt === 'jpg' ? 'jpeg' : imgExt;
              result.thumbnail = `data:image/${mime};base64,${data.toString('base64')}`;
              result.thumbnailSource = 'folder';
              break;
            } catch {}
          }
        }
      }
      const stats = getResourceTypeStats(buf);
      result.resourceTypes = stats.types;
      result.resourceTotal = stats.total;
    } else if (ext === '.ts4script') {
      result.resourceTypes = [{ type: 'ts4script', name: 'Python 脚本模组', count: 1 }];
      result.resourceTotal = 1;
    }
    return result;
  } catch (e) {
...
function toggleDisabled(filePath, enable) {
  if (!filePath) return { ok: false, error: '未提供路径' };
  let target = filePath;
  const base = path.basename(filePath);
  const dir = path.dirname(filePath);
  const isDisabled = base.toLowerCase().endsWith('.disabled');
  if (enable && isDisabled) {
    target = path.join(dir, base.slice(0, -'.disabled'.length));
  } else if (!enable && !isDisabled) {
    target = path.join(dir, base + '.disabled');
  } else {
    return { ok: true, path: filePath, noChange: true };
  }
  try {
    if (fs.existsSync(target)) {
      return { ok: false, error: '目标文件名已存在', path: filePath };
    }
    fs.renameSync(filePath, target);
    opLog(enable ? 'ENABLE_MOD' : 'DISABLE_MOD', `${filePath} -> ${target}`);
...
ipcMain.handle('toggle-mod-batch', async (event, paths, enable) => {
  if (!Array.isArray(paths)) return { ok: false, error: '参数错误' };
  const results = [];
  for (const p of paths) {
  ...
  return new Promise((resolve) => {
    ...
    if (process.platform === 'win32') {
      if (ext === '.zip') {
        ...
        cmd = `powershell -NoProfile -Command "Expand-Archive -Path '${archivePath.replace(/'/g, "''")}' -DestinationPath '${destDir.replace(/'/g, "''")}' -Force"`;
      } else {
        // 尝试 7z
    ...
    if (!cmd) return resolve({ ok: false, error: '不支持的压缩格式' });
    exec(cmd, { timeout: 120000 }, (err, stdout, stderr) => {
      if (err) {
        resolve({ ok: false, error: `解压失败（可能缺少 7z/unrar 工具）：${err.message}` });
      } else {
        resolve({ ok: true });
      }
    });
...
// 递归收集目录下的 .package/.ts4script 文件（过滤非 MOD 文件）
async function collectModFiles(dir) {
  const result = [];
  await walkDir(dir, (file) => {
    const name = path.basename(file).toLowerCase();
    if (name.endsWith('.package') || name.endsWith('.ts4script')) {
      result.push(file);
    }
  });
  return result;
...
// 根据文件名自动判断分类
function guessCategoryForFile(name) {
  const entry = { name, relPath: '' };
  return autoClassifyFile(entry);
}

ipcMain.handle('import-dropped-files', async (event, filePaths) => {
  if (!state.modsFolder) return { ok: false, error: '未设置 Mods 文件夹' };
  if (!Array.isArray(filePaths) || filePaths.length === 0) return { ok: false, error: '未提供文件' };
  const imported = [];
  const errors = [];
    ...
    try {
      ...
      if (stat.isFile()) {
        ...
        if (isArchiveFile(lowerName)) {
          // 解压到临时目录
          const tmpDir = path.join(require('os').tmpdir(), 'sims4ycc_import_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8));
          fs.mkdirSync(tmpDir, { recursive: true });
          const r = await extractArchive(src, tmpDir);
          if (!r.ok) { errors.push({ file: name, error: r.error }); continue; }
          const modFiles = await collectModFiles(tmpDir);
          for (const mf of modFiles) {
            const cat = guessCategoryForFile(path.basename(mf));
            const targetDir = path.join(state.modsFolder, ...cat);
            fs.mkdirSync(targetDir, { recursive: true });
            const targetPath = path.join(targetDir, path.basename(mf));
            if (!fs.existsSync(targetPath)) {
              fs.copyFileSync(mf, targetPath);
              imported.push({ from: mf, to: targetPath, category: cat });
            }
          }
          // 清理临时目录
          try { fs.rmSync(tmpDir, { recursive: true, force: true }); } catch {}
        } else if (lowerName.endsWith('.package') || lowerName.endsWith('.ts4script')) {
          const cat = guessCategoryForFile(name);
          const targetDir = path.join(state.modsFolder, ...cat);
          fs.mkdirSync(targetDir, { recursive: true });
          const targetPath = path.join(targetDir, name);
          if (!fs.existsSync(targetPath)) {
            fs.copyFileSync(src, targetPath);
            imported.push({ from: src, to: targetPath, category: cat });
          } else {
            errors.push({ file: name, error: '目标文件已存在' });
          }
        } else {
          // 非 MOD 文件，跳过
          errors.push({ file: name, error: '非 MOD 文件，已跳过' });
        }
      } else if (stat.isDirectory()) {
        // 整个文件夹：收集其中 MOD 文件按分类导入
        const modFiles = await collectModFiles(src);
        for (const mf of modFiles) {
          const cat = guessCategoryForFile(path.basename(mf));
          const targetDir = path.join(state.modsFolder, ...cat);
          fs.mkdirSync(targetDir, { recursive: true });
  ...
  opLog('IMPORT_DROP', `导入 ${imported.length} 个文件，错误 ${errors.length} 个`);
  return { ok: true, imported, errors, importedCount: imported.length };
});

// ============ 功能五：自动定位 Mods 文件夹 ============
function getDocumentsDir() {
  // Windows: C:\Users\<user>\Documents 或 OneDrive 重定向
  // macOS: ~/Documents
  const home = require('os').homedir();
  const candidates = [];
  if (process.platform === 'win32') {
    candidates.push(path.join(home, 'Documents'));
    candidates.push(path.join(home, 'OneDrive', 'Documents'));
    candidates.push(path.join(home, 'OneDrive - Personal', 'Documents'));
    // 注册表 USER_SHELL_FOLDERS 可能更准确，但跨平台简化处理
  } else {
    candidates.push(path.join(home, 'Documents'));
    candidates.push(path.join(home, 'Library', 'Containers', 'com.aspyr.sims4', 'Data', 'Library', 'Application Support', 'Sims 4'));
  }
  for (const c of candidates) {
...
ipcMain.handle('auto-locate-mods-folder', async () => {
  const docDir = getDocumentsDir();
  // Sims 4 常见安装路径（含本地化）
  const sims4Names = ['The Sims 4', 'Los Sims 4', 'Les Sims 4', 'Die Sims 4', 'Sims 4', '模拟市民4'];
  const found = [];
  for (const name of sims4Names) {
    const modsPath = path.join(docDir, name, 'Mods');
    if (fs.existsSync(modsPath)) {
      found.push({ path: modsPath, source: 'Documents/' + name });
    }
  }
  // Ultimate Collection / 多安装：扫描 Documents 下含 Mods 子目录且名为 *Sims* 的文件夹
  try {
    const entries = fs.readdirSync(docDir, { withFileTypes: true });
    for (const e of entries) {
      if (!e.isDirectory()) continue;
      if (/sims/i.test(e.name)) {
        const modsPath = path.join(docDir, e.name, 'Mods');
        if (fs.existsSync(modsPath) && !found.find(f => f.path === modsPath)) {
          found.push({ path: modsPath, source: 'Documents/' + e.name });
        }
      }
    }
  } catch {}
  ...
  if (process.platform === 'darwin') {
    const appStore = path.join(require('os').homedir(), 'Library', 'Containers', 'com.aspyr.sims4', 'Data', 'Library', 'Application Support', 'Sims 4', 'Mods');
    if (fs.existsSync(appStore) && !found.find(f => f.path === appStore)) {
      found.push({ path: appStore, source: 'macOS App Store' });
    }
  }
  if (found.length > 0) {
    // 自动选择第一个
    state.modsFolder = found[0].path;
    saveState();
    return { found: true, candidates: found, selected: found[0].path };
  }
  return { found: false, candidates: [], message: '未自动找到 Mods 文件夹，请手动选择或一键创建' };
});

// ============ 功能六：一键创建 Mods 文件夹 + Resource.cfg ============
const RESOURCE_CFG_CONTENT = `Priority 500
PackedFile *.package
PackedFile */*.package
PackedFile */*/*.package
PackedFile */*/*/*.package
...
ipcMain.handle('create-mods-folder', async () => {
  const docDir = getDocumentsDir();
  const sims4Dir = path.join(docDir, 'The Sims 4');
  const modsDir = path.join(sims4Dir, 'Mods');
  try {
    fs.mkdirSync(modsDir, { recursive: true });
    // 创建 Resource.cfg
    const cfgPath = path.join(modsDir, 'Resource.cfg');
    if (!fs.existsSync(cfgPath)) {
      fs.writeFileSync(cfgPath, RESOURCE_CFG_CONTENT, 'utf8');
    }
    // 创建默认分类子文件夹
    for (const cat of defaultCategories()) {
      const catDir = path.join(modsDir, cat.name);
      fs.mkdirSync(catDir, { recursive: true });
    }
    state.modsFolder = modsDir;
    saveState();
    opLog('CREATE_MODS_FOLDER', `创建 Mods 文件夹：${modsDir}`);
...
// 根据标签推断分类路径：如果标签匹配某个分类名，则使用该分类
function inferCategoryFromTags(tags, categories) {
  const leafMap = collectCategoryLeaves(categories || defaultCategories());
  for (const tag of (tags || [])) {
    if (leafMap[tag]) return leafMap[tag];
  }
  return null;
}
