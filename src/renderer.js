...
function bindTopbar() {
  ...
  // 功能五：自动定位 Mods 文件夹
  const btnAutoLocate = $('#btnAutoLocate');
  if (btnAutoLocate) {
    btnAutoLocate.addEventListener('click', async () => {
      btnAutoLocate.disabled = true;
      btnAutoLocate.textContent = '定位中...';
      try {
        const r = await api.autoLocateModsFolder();
        if (r.found) {
          appState.modsFolder = r.selected;
          $('#modsPathDisplay').textContent = appState.modsFolder;
          let msg = `已自动定位到：${r.selected}`;
          if (r.candidates.length > 1) {
            msg += `（共发现 ${r.candidates.length} 个 Mods 文件夹，已选择第一个）`;
          }
          toast(msg, 'success');
          renderOverview();
        } else {
          toast(r.message || '未找到 Mods 文件夹', 'error');
        }
      } catch (e) {
        toast('自动定位失败：' + e.message, 'error');
      } finally {
        btnAutoLocate.disabled = false;
        btnAutoLocate.textContent = '自动定位';
      }
    });
  }

  // 功能六：一键创建 Mods 文件夹
  const btnCreateMods = $('#btnCreateMods');
  if (btnCreateMods) {
    btnCreateMods.addEventListener('click', async () => {
      if (!confirm('将在文档目录下创建 "The Sims 4/Mods" 文件夹，并生成 Resource.cfg 和默认分类子文件夹。是否继续？')) return;
      btnCreateMods.disabled = true;
      btnCreateMods.textContent = '创建中...';
      try {
        const r = await api.createModsFolder();
        if (r.ok) {
          appState.modsFolder = r.path;
          $('#modsPathDisplay').textContent = appState.modsFolder;
          toast('Mods 文件夹已创建：' + r.path, 'success');
          renderOverview();
        } else {
        ...
        btnCreateMods.disabled = false;
        btnCreateMods.textContent = '一键创建';
      }
    });
  }
...
function bindDragAndDrop() {
  ...
  window.addEventListener('drop', async (e) => {
    ...
    if (paths.length === 0) {
      toast('未获取到文件路径', 'error');
      return;
    }
    toast('正在导入...');
    try {
      const r = await api.importDroppedFiles(paths);
      if (r.ok) {
        toast(`导入完成：成功 ${r.importedCount} 个，失败 ${r.errors.length} 个`, r.importedCount > 0 ? 'success' : 'error');
        if (r.errors.length > 0) {
          console.log('导入失败项：', r.errors);
        }
      } else {
...
async function openModDetail(filePath) {
  ...
  $('#modDetailTitle').textContent = 'MOD 详情（加载中...）';
  $('#modDetailName').textContent = basename(filePath);
  $('#modDetailThumb').style.display = 'none';
  $('#modDetailNoThumb').style.display = 'flex';
  ...
  try {
    ...
    $('#modDetailTitle').textContent = 'MOD 详情';
    if (r.thumbnail) {
      const img = $('#modDetailThumb');
      img.src = r.thumbnail;
      img.style.display = 'block';
      $('#modDetailNoThumb').style.display = 'none';
      const srcText = r.thumbnailSource === 'package' ? '预览图源：.package 内置缩略图' : '预览图源：同目录图片';
      $('#modDetailThumbSource').textContent = srcText;
    } else {
      $('#modDetailThumb').style.display = 'none';
      $('#modDetailNoThumb').style.display = 'flex';
      $('#modDetailThumbSource').textContent = '';
    ...
    if (r.resourceTypes && r.resourceTypes.length > 0) {
      $('#modDetailResources').innerHTML = r.resourceTypes.map(rt =>
        `<span class="resource-type-chip" title="0x${rt.type}">${esc(rt.name)} <em>×${rt.count}</em></span>`
      ).join('');
    } else {
      $('#modDetailResources').innerHTML = '<span class="zh-name-placeholder">未解析到资源类型</span>';
    }
    const meta = `资源总数：${r.resourceTotal}`;
    $('#modDetailMeta').textContent = filePath + ' · ' + meta;
  } catch (e) {
...
function renderConflicts() {
  ...
  area.innerHTML = conflictList.map((c, gi) => {
    ...
    <div class="conflict-item ${c.hasAnchored ? 'has-anchored' : ''}" data-group-idx="${gi}">
      ...
      ${c.type === 'version' ? `<div class="version-hint">⚠ 检测到同一 MOD 的多个版本同时存在，这是导致 CC 异常的常见原因，请仅保留最新版本。</div>` : ''}
      ${c.type === 'duplicate' ? `<div class="version-hint">⚠ 同一文件被重复安装在不同位置，建议删除多余副本。</div>` : ''}
      ${c.sameAuthor ? `<div class="same-author-hint">💡 两个冲突文件作者相同，很可能是同一作者发布的不同版本更迭，建议保留日期最新的那个。</div>` : ''}
      ${c.resourceCoverage ? `
        <div class="resource-coverage">
          <strong>资源覆盖明细：</strong>
          <div class="coverage-list">
            ${c.resourceCoverage.map(rc => `
              <div class="coverage-item">
                <span class="coverage-file">${esc(rc.name)}</span>
                <span class="coverage-count">覆盖 ${rc.keyCount} 个冲突资源</span>
              </div>
            `).join('')}
          </div>
        </div>
        ...
        ${c.files.map((f, idx) => {
          ...
          <div class="conflict-file ${disabledClass}" data-group-idx="${gi}">
            ...
            <div style="flex:1;min-width:0">
              ...
              ${f.version ? `<span class="version-badge" title="从文件名识别的版本号">v${esc(f.version)}</span>` : ''}
...
function bindClassifyPage() {
  ...
  const btnBatchDisable = $('#btnBatchDisable');
  if (btnBatchEnable) {
    btnBatchEnable.addEventListener('click', async () => {
      if (appState.selectedFiles.size === 0) { toast('请先选择文件', 'error'); return; }
...
async function applyClassification() {
  ...
  for (const p of appState.selectedFiles) {
    ...
    // 主进程会根据标签推断分类并返回 tagCorrected
    const res = await api.setClassification(p, payload) || {};
    if (res.tagCorrected && res.category) {
  ...
  let msg = `已应用到 ${applied} 个文件（标记为"手动调整"）`;
